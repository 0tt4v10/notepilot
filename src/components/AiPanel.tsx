...
const MODES: { id: Mode; label: string; emoji: string }[] = [
  { id: 'summary', label: 'Zusammenfassung', emoji: '📝' },
  { id: 'questions', label: 'Prüfungsfragen', emoji: '❓' },
  { id: 'gaps', label: 'Lücken erkennen', emoji: '🔍' },
...
export default function AiPanel({ noteText, onClose }: Props) {
  ...
  const runMode = async (m: Mode) => {
    if (!plainText) { setError('Die Notiz ist leer.'); return; }
    setMode(m);
  ...
  return (
    <div className="w-80 border-l border-slate-200 flex flex-col bg-white">
      {/* Header */}
      <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between bg-gradient-to-r from-blue-50 to-purple-50">
        <div className="flex items-center gap-2">
          <Sparkles size={16} className="text-blue-500" />
          <span className="text-sm font-semibold text-slate-700">KI-Assistent</span>
        </div>
        ...
        {mode !== 'chat' ? (
          ...
          <div className="flex flex-col h-full">
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              {chatMsgs.length === 0 && (
                <p className="text-slate-400 text-sm text-center mt-6">
                  Stell mir eine Frage zu deiner Notiz.
                </p>
              )}
              {chatMsgs.map((msg, i) => (
                <div
              ...
              {loading && (
                <div className="bg-slate-100 rounded-lg px-3 py-2 mr-6 flex items-center gap-2 text-slate-400 text-sm">
            ...
            {chatMsgs.length > 0 && (
              <button
                onClick={() => setChatMsgs([])}
                className="mx-3 mb-1 flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600"
              >
                <RotateCcw size={11} /> Gespräch zurücksetzen
              </button>
            )}
            <div className="p-3 border-t border-slate-200 flex gap-2">
              <input
                ...
                disabled={loading}
                className="flex-1 text-sm px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50"
              />
              <button
                onClick={sendChat}
